...
const analysisSchema = { type: 'object', additionalProperties: false, required: ['events','places'], properties: {
  events: { type: 'array', items: { type: 'object', additionalProperties: false, required: ['order','type','time','title','facts','place','category','media_ids'], properties: { order:{type:'integer'}, type:{type:'string'}, time:{type:['string','null']}, title:{type:'string'}, facts:{type:'string'}, place:{type:['string','null']}, category:{type:'string'}, media_ids:{type:'array',items:{type:'string'}} } } },
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const analysis = await structured('travel_day_analysis', 'Extrais uniquement les faits du récit. Crée une chronologie. Les lieux sont des candidats à confirmer : ne les invente jamais. Associe les médias seulement si leur nom fournit un indice fiable.', context, analysisSchema);
    ...
    if (journal.events.length) await admin.from('journal_events').insert(journal.events.map((e:any)=>({journal_id:saved.id,event_order:e.order,event_type:e.type,event_time:e.time||null,title:e.title,description:e.description,place_text:e.place,category:e.category})));
    ...
    return Response.json({ ...journal, placeCandidates:candidateRows, model:MODEL }, { headers:cors });
  } catch (error) {
