...
Deno.serve(async(req)=>{if(req.method==='OPTIONS')return new Response('ok',{headers:cors});try{
  ...
  const{data:publication}=await admin.from('trip_publications').select('snapshot').eq('trip_id',tripId).single();if(publication){const days=(publication.snapshot.days??[]).map((day:any)=>({...day,layout:layouts?.find((row:any)=>row.trip_days.day_date===day.date)?.layout??'editorial'}));await admin.from('trip_publications').update({snapshot:{...publication.snapshot,design:settings.design??'editorial',photos:photoUrls,days}}).eq('trip_id',tripId);}
