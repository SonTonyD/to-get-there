...
-- Schéma d'initialisation uniquement : à exécuter sur une base vide.
-- Sur une base existante, exécuter seulement les fichiers de supabase/migrations dans l'ordre chronologique.
-- Les comptes vivent dans auth.users.
...
create table public.day_journals (
  ...
  cover_media_id uuid,
  design_settings jsonb not null default '{"style":"wanderlust","palette":"candy","paper":"grid","composition":"collage","font":"handwritten","decorations":"balanced"}'::jsonb,
...
alter table public.day_journals add column if not exists design_settings jsonb not null default '{"style":"wanderlust","palette":"candy","paper":"grid","composition":"collage","font":"handwritten","decorations":"balanced"}'::jsonb;
